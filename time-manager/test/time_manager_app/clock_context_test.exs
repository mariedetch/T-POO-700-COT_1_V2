defmodule TimeManagement.ClockContextTest do
  ...
  describe "clocks" do
    ...
    import TimeManagement.ClockContextFixtures
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2024-10-07 15:33:00]}

      assert {:ok, %Clock{} = clock} = ClockContext.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2024-10-07 15:33:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2024-10-08 15:33:00]}

      assert {:ok, %Clock{} = clock} = ClockContext.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2024-10-08 15:33:00]
    end
