...
const validateFields = () => {
  ...
  errors.value.email = user.value.email
    ? /\S+@\S+\.\S+/.test(user.value.email as string)
      ? ''
...
const onSubmit = async () => {
  if (validateFields()) {
    try {
      ...
      ToastrService.error(
        `Echec de création ${selectedUser.value ? 'modification' : 'création'} de l\'utilisateur`
      )
    } finally {
      onCloseModal()
    }
    ...
    <div class="grid grid-cols-12 gap-6">
      <div class="col-span-12">
        <div class="card table-card">
          ...
          <div class="card-body">
            <UsersList
              @edit-user="onEditModalOpen"
              @remove-user="showDeleteConfirmation"
              :users="users"
            />
          </div>
        </div>
    ...
    <Modal :isOpened="isModalOpened" modalId="createUserModal" :modalTitle="modalData.title">
      <form @submit.prevent="onSubmit">
        ...
        <div class="modal-footer">
          <button @click="onCloseModal()" type="button" class="btn btn-secondary">Close</button>
          <button
            v-if="isLoading"
